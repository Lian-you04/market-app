...
def market_durum_guncelle():
    try:
        ...
        if "aktif" in data: market.aktif = bool(data["aktif"])
        if "min_siparis_tutari" in data: market.min_siparis_tutari = float(data["min_siparis_tutari"])
        if "maks_teslimat_km" in data: market.maks_teslimat_km = float(data["maks_teslimat_km"])
        if "konum_lat" in data: market.konum_lat = float(data["konum_lat"])
        if "konum_lon" in data: market.konum_lon = float(data["konum_lon"])
        if "adres" in data: market.adres = str(data["adres"]).strip()

        db.session.commit()
        
        # 🚀 CANLI SİNYAL: Bakkal dükkanı açıp kapattığında vitrindeki müşterilerin ekranı anında değişsin!
        socketio.emit("market_durumu_degisti", {"market_id": market.id, "aktif": market.aktif})
...
def urun_ekle():
    try:
        ...
        resim_yolu = ",".join(kaydedilen_yollar) if kaydedilen_yollar else form_verisi.get("resim_url", "https://images.unsplash.com/photo-1583258292688-d0213dc5a3a8?auto=format&fit=crop&w=300&q=80")
...
def urun_guncelle(urun_id):
    try:
        ...
        data = request.json or {}
        if "fiyat" in data: urun.fiyat = float(data["fiyat"])
        if "stok_adet" in data:
            yeni_stok = int(data["stok_adet"])
            urun.stok_adet = yeni_stok
            urun.aktif = yeni_stok > 0
        if "aktif" in data: urun.aktif = bool(data["aktif"])
        if "ad" in data: urun.ad = str(data["ad"])
        if "aciklama" in data: urun.aciklama = str(data["aciklama"])
        db.session.commit()
        return jsonify({"mesaj": "Ürün güncellendi", "stok": urun.stok_adet, "aktif": urun.aktif})
    except Exception as e:
        db.session.rollback()
        return jsonify({"hata": str(e)}), 500
...
def urunler_listele():
    try:
        ...
        return jsonify([{
            "id": u.id, "ad": u.ad, "aciklama": u.aciklama,
            "fiyat": float(u.fiyat), "stok_adet": u.stok_adet,
            "kategori": u.kategori, "resim_url": u.resim_url, "aktif": u.aktif
        } for u in urunler])
    except Exception as e:
        return jsonify({"hata": str(e)}), 500
...
def siparisler_listele():
    try:
        market_id = request.args.get("market_id", 1, type=int)
        siparisler = Siparis.query.filter_by(market_id=market_id).order_by(Siparis.olusturma_tarihi.desc()).all()
...
def siparis_kalem_duzenle(siparis_id, detay_id):
    try:
        ...
        if yeni_adet < 0: return jsonify({"hata": "Adet negatif olamaz!"}), 400
...
def siparis_durum_guncelle(siparis_id):
    try:
        ...
        db.session.commit()
        socketio.emit("siparis_durumu_degisti", {"siparis_id": siparis.id, "yeni_durum": yeni_durum})
        return jsonify({"mesaj": f"Durum '{yeni_durum}' yapıldı"})
    except Exception as e:
        db.session.rollback()
        return jsonify({"hata": str(e)}), 500